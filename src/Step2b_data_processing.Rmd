...
#### Created tabulated file for counts per sequence read
```{bash}
...
```{r}
merged_files <- list.files(merged_dir, pattern="*.fastq", full.names = T)
```

```{r}
control_files <- merged_files[grepl(paste(control_samples, collapse= "|"), merged_filess)]
```
