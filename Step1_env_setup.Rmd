...
pacman::p_load(knitr, here, tidyr, dplyr, readxl, stringr, spgs, filesstrings, lubridate)
...
Update the `samples_path` to `here(params$datetime, "test_data")` if using your own data and the `sample_sheet` to `here("sample_sheet_test.xlsx")`. Also update the path in the file copy (line 66) to `from=here("test_data")`. Similarly, update the path in the file copy (line 67) to `from=here("sample_sheet_test.xlsx")`.

```{r}
dir.create(here(params$datetime))
work_dir <- file.path(here(params$datetime))
src_path <- file.path(here(params$datetime, "src"))
samples_path <- file.path(here(params$datetime, "data")) # edit to use your own data, see text above
...
sample_sheet <- read_xlsx(here("sample_sheet_test.xlsx"), col_types = "guess") # edit to use the test data, see text above
```
...
file.copy(from=here("template/data"), to=here(params$datetime), recursive=TRUE)
file.copy(from=here("sample_sheet.xlsx"), to=here(params$datetime))
```
...
**Note:** View your `sample_sheet` to make sure all the dates are displayed as `yyyy-mm-dd`. If you see NAs, go back to the `xlsx` form and enter them either as `mm/dd/yyyy` or `mm-dd-yyyy` format.