...
for sample in $(cat $sample_path/sampleNames)
do
...
```{r}
merged_files <- list.files(here(params$today, "results/merged"), pattern="*id.fastq", full.names = T)
```
...
Add all of the `*id.fastq` files you want to move to the list named `samples_to_move`. This will move them to a `bad_controls_samples` folder. These can be from suspected contamination or from low read counts (i.e., < 1000 reads).
```{r}
samples_to_move <- c("", "")
```
...
move_files(samples_to_move, bad_controls_path, overwrite = FALSE)
```
