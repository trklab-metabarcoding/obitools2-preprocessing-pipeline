...
pacman::p_load(knitr, here, tidyr, dplyr, readxl, stringr, spgs, filesstrings, beepr)
```
...
```{r}
beep(1) #audio notification upon process completion
#try integers 1-11 for different sounds #(https://www.r-project.org/nosvn/pandoc/beepr.html)
```
...
```{r}
beep() # audio notification upon completion
```
