...
``
#### Generate some derived parameters from the Step1 inputs
...
```{r}
file_names_original <- list.files(samples_path, pattern=".gz", full.names = T)
...
```{r}
control_samples <- sample_sheet %>% filter(Control == "Y")
control_samples <- unique(control_samples$SampleID)
```

```{r}
capture.output(control_samples, file = here(params$today, 'results/control_list.txt'))
capture.output(file_names, file = here(params$today, 'results/file_names.txt'))
```
...
source ~/.bash_profile
conda create -n cutadaptenv -c conda-forge -c bioconda cutadapt python=3.9
```
...
do
    ...
    cutadapt -a ^$F_primer...$R_primer_reverse -A ^$R_primer...$R_primer_reverse --no-indels -e $primer_mismatch --discard-untrimmed -o ${sample}_L001_R1_001_trimmed.fastq.gz -p ${sample}_L001_R2_001_trimmed.fastq.gz ${sample}_L001_R1_001.fastq.gz ${sample}_L001_R2_001.fastq.gz >> cutadapt_primer_trimming_stats.txt 2>&1
done

mv *trimmed.fastq.gz $output_path
```
...
paste sampleNames <(grep "passing" $output_path/cutadapt_primer_trimming_stats.txt | cut -f3 -d "(" | tr -d ")") <(grep "filtered" $output_path/cutadapt_primer_trimming_stats.txt | cut -f3 -d "(" | tr -d ")")
```
