...
output: 
  ...
  toc: true
params:
    step1_analysis_date: !r (format(Sys.time(), "%Y%m%d"))
---
```{r include=FALSE}
...
Sys.setenv(analysis_date=params$step1_analysis_date)
```
...
destination=${data_path}/${project_code}/${mapped_folder}/${DIR}/${analysis_date}
mkdir -p ${destination}
cp -r ./results/id_data ${destination}
cp -r ./results/bad_controls_path ${destination}
cp -r ./renamed_data ${destination}
cp *.Rmd ${destination}
cp cutadapt_primer_trimming_stats.txt ${destination}
